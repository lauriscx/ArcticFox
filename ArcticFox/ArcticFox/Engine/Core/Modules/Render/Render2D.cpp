...
void Graphics::Render2D::DrawQuad(const glm::vec3 & position, const glm::vec2 & size, const glm::vec4 & color, Texture * texture, float rotate) {
	if (s_2DRenderData->QuadIndexCount >= Render2DCache::maxIndices) {
		EndScene();
	}
...
void Graphics::Render2D::ResetStats() {
	s_2DRenderData->m_Statistics.m_DrawCalls = 0;
	s_2DRenderData->m_Statistics.m_QuodCount = 0;
}

Graphics::Render2D::Statistics Graphics::Render2D::GetStats() {
